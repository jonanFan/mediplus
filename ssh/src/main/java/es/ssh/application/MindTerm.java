...
//    (new Thread(this)).start();
  ...
  public void run() {
	...
	if(termProps != null && !termProps.isEmpty()) {
	  Enumeration enuma = termProps.keys();
...
//	  clientThread = new Thread(client);
	if(SSH.NETSCAPE_SECURITY_MODEL) {
	  ...
	  clientThread = new Thread(SSH.getThreadGroup(), client); // JH_Mod
	  clientThread.start();
	  clientThread.join();
	} catch(InterruptedException e) {
	  // !!!
...
//    (new Thread(mindterm)).start();
    ...
    (new Thread(SSH.getThreadGroup(), mindterm)).start(); // JH_Mod
  }
...
//    (new Thread(mindterm)).start();
  ...
  public void exit() {
    if(!confirmClose())
      return;
    Enumeration enuma = terminals.elements();
    while(enuma.hasMoreElements()) {
      MindTerm mt = (MindTerm)enuma.nextElement();
