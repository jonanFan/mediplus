...
public class ReminderCiteActivity extends Activity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cite_reminder);
    }
    ...
    protected void onStart() {
        ...
        if (postponedTimes==ReminderManager.getReminderRepetitions()) {
            ...
            cita = (Cita) extras.getSerializable(ReminderManager.EXTRA_CITE);

            TextView citeNameView = (TextView) findViewById(R.id.activity_reminder_cite_name);
            citeNameView.setText(cita.getEventName());

            TextView citeDescrView = (TextView) findViewById(R.id.activity_reminder_cite_description);
            citeDescrView.setText(cita.getDescription());

            TextView citeLocationView = (TextView) findViewById(R.id.activity_reminder_cite_location);
            citeLocationView.setText(cita.getLocation());
            ...
            View buttonDone = findViewById(R.id.button_cite_done);
            View buttonPostpone = findViewById(R.id.button_cite_postpone);
            View buttonNotWanna = findViewById(R.id.button_cite_not_wanna);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.button_cite_done) {
            Log.d(TAG, "onClick en button_done");
            vibrator.cancel();
            asyncRingtonePlayer.stop();

            Toast.makeText(this, R.string.cite_gone, Toast.LENGTH_LONG).show();

            if (postponedTimes!= 0) {
                ReminderManager.cancelPostponed(this,cita);
            }

            this.finish();
        }
        else if (v.getId() == R.id.button_cite_not_wanna) {
            Log.d(TAG, "onClick en button_not_wanna");
            vibrator.cancel();
            asyncRingtonePlayer.stop();

            Toast.makeText(this, R.string.cite_cancelled, Toast.LENGTH_LONG).show();
            ...
            if (postponedTimes==0)
                ReminderManager.postponeReminder(this, cita, ReminderManager.getPostponeTimeMilis());

            Toast.makeText(this, R.string.cite_postponed, Toast.LENGTH_LONG).show();
